...
class TableOfContents extends AbstractBlock {
	...
	protected function get_default_html_attrs() {
		return [
			...
			'class'          => $this->cn(
				"blockart-toc blockart-toc-{$this->get_attribute( 'clientId', '' )}",
	...
	public function build_html( $content ) {
		...
		ob_start();
		?>
		<script>var _blockart_toc_<?php echo esc_js( $this->get_attribute( 'clientId', '' ) ); ?> = <?php echo wp_json_encode( $headings ); ?>;</script>
