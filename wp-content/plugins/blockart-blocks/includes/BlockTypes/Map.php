...
class Map extends AbstractBlock {
	...
	public function build_html( $content ) {
		...
		?>
		<?php if ( $has_api_key ) : ?>
			<script>var _blockart_map_<?php echo esc_js( $this->get_attribute( 'clientId', '' ) ); ?> = <?php echo wp_json_encode( $this->get_map_options() ); ?>;</script>
		<?php endif; ?>
		<div <?php $this->build_html_attributes( true ); ?>>
			<?php if ( ! $has_api_key ) : ?>
				<div class="blockart-map-iframe">
					<iframe src="<?php echo esc_url( $google_maps_url ); ?>" width="100%" height="<?php echo esc_attr( $map_height ); ?>" style="border:none;"></iframe>
				</div>
			<?php endif; ?>
		</div>
		<?php
